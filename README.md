...
- **무손실 변환** — 원본 XML의 모든 요소·속성·텍스트가 AST에 그대로 보존됩니다(116개 자전거 샘플 데이터셋, 24개 스키마, 236개 회귀 테스트로 검증).
...
데이터셋이 없으면 sweep 명령은 비활성화되며, 회귀 테스트는 자동으로 skip되므로 코어 엔진 자체는 그대로 동작합니다.
